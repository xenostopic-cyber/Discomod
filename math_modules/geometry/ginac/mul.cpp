...
GINAC_IMPLEMENT_REGISTERED_CLASS_OPT(mul, expairseq,
  ...
  print_func<print_sortex>(&mul::do_print_sortex).
...
void mul::do_print_sortex(const print_sortex & c, unsigned level) const
{
	if (precedence() <= level)
		c.s << '(';
	...
	// All expairs, keyed by their rest's sortex-printed string
	std::multimap<std::string, expair> sorted_seq;
	for (auto& it : seq) {
		sorted_seq.emplace((std::stringstream{} << sortex << it.rest).str(), it);
	}

	bool first = true;
	for (auto& it : sorted_seq) {
		if (!first)
			c.s << '*';
		else
			first = false;
		recombine_pair_to_ex(it.second).print(c, precedence());
	}
...
void mul::do_print_latex(const print_latex & c, unsigned level) const
{
	...
	// All expairs, keyed by their rest's latex-printed string
	std::multimap<std::string, expair> sorted_seq;
	for (auto& it : seq) {
		sorted_seq.emplace((std::ostringstream{} << latex << it.rest).str(), it);
	}
	...
	for (auto& it : sorted_seq) {
		GINAC_ASSERT(is_exactly_a<numeric>(it.second.coeff));
		if (ex_to<numeric>(it.second.coeff).is_negative())
			neg_powers.push_back(recombine_pair_to_ex(expair(it.second.rest, -it.second.coeff)));
		else
			others.push_back(recombine_pair_to_ex(it.second));
	}
