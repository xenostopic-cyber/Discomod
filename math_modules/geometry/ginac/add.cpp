...
GINAC_IMPLEMENT_REGISTERED_CLASS_OPT(add, expairseq,
  ...
  print_func<print_sortex>(&add::do_print_sortex).
...
void add::print_add_sorted(const print_context& c, const char* openbrace, const char* closebrace, const char* mul_sym, unsigned level) const
{
	if (precedence() <= level)
		c.s << openbrace << '(';
	...
	for (auto& it : seq) {
		sorted_seq.emplace((std::ostringstream{} << sortex << it.rest).str(), it);
	}

	// Then proceed with the remaining factors
	for (auto& it : sorted_seq) {
		coeff = ex_to<numeric>(it.second.coeff);
		if (!first) {
			if (coeff.csgn() == -1) c.s << '-'; else c.s << '+';
		} else {
			if (coeff.csgn() == -1) c.s << '-';
		...
		it.second.rest.print(c, precedence());
	}
...
void add::do_print_sortex(const print_sortex & c, unsigned level) const
{
	print_add_sorted(c, "", "", "*", level);
}
