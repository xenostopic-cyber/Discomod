...
next_context:
	...
	if (id >= pdt.size() || !(pdt[id].is_valid())) {
		...
		// registry.
		throw std::runtime_error("basic::print(): method for "s + class_name() + "/" + c.class_name() + " not found");
...
{
	throw std::range_error("basic::op(): "s + class_name() + " has no operands");
}
...
ex & basic::let_op(size_t i)
{
	...
	throw std::range_error("basic::let_op(): "s + class_name() + " has no operands");
}
...
ex basic::operator[](const ex & index) const
{
	...
	throw std::invalid_argument("non-numeric indices not supported by "s + class_name());
}
