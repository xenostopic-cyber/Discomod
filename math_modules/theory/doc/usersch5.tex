...
These routines convert nonnegative lengths (often attached to floating point
numbers) between different units. Their behavior is undefined at negative
integers. Strictly speaking the \emph{length} of $x$ is the output of
\kbd{lg}$(x)$ and counts a number of words, themselves comprising $\B$ bits.
By extension, lengths are also expressed in bits or even in decimal digits
but we will not call these ``length'' in this document.

A \emph{precision} is restricted to floating point numbers and the \typ{REAL}
type; it is the output of \kbd{realprec}$(x)$, usually called \kbd{prec}, and
counts the number of bits in the mantissa. For historical and practical
reasons, \kbd{prec}s are multiple of $\B$. More general bit counts are called
``number of bits'' (\kbd{nbits}) or ``bit precision''.

We can convert a length to a bit precision without losing information, and
the result is a \kbd{prec}. In the other direction, the conversion from bits
to length rounds to the next multiple of $\B$ and loses information: the
original bit count can only be recovered with an error in $[0,B-1[$. There is
no error involved if the bit count is known to be a \kbd{prec}.

\fun{long}{ndec2nlong}{long x} converts a number of decimal digits to a number
of words. Returns $ 1 + \kbd{floor}(x \log_2 10 / \B)$.

\fun{long}{ndec2nbits}{long x} converts a number of decimal digits to a
number of bits. This is \kbd{ndec2nlong}$(x)$ times $\B$.

\fun{long}{ndec2prec}{long x} converts a number of decimal digits to a
precision. This is an alias for \kbd{ndec2nbits}.

\fun{long}{nbits2ndec}{long x} converts a number of bits to a number of
decimal digits. Returns \kbd{floor}$(x \log_{10} 2)$.
...
of length \kbd{x}. Currently an alias for \kbd{bit\_accuracy}.
...
\fun{long}{bit_prec}{GEN x} deprecated alias for \kbd{realprec}.

\fun{long}{nbits2extraprec}{long x} convert $x$ to a valid precision. For
...
number of decimal digits. This is an alias for \kbd{nbits2ndec}.

\fun{long}{prec2lg}{long x} return the length of a \typ{REAL} of precision
\kbd{x}. This is an alias for \kbd{nbits2lg}).
...
\item if \kbd{x} is not zero: the bit size of its mantissa;

\item if \kbd{x} is numerically equal to $0$, of exponent $e$:
\kbd{nbits2prec}$(-e)$ if $e < 0$ and \kbd{LOWDEFAULTPREC} if $e \geq 0$.
...
\noindent \kbd{long} \tet{DEFAULTPREC}:    the precision (\kbd{realprec}) of a
\typ{REAL} with 64 bits of accuracy

\noindent \kbd{long} \tet{MEDDEFAULTPREC}: the precision (\kbd{realprec}) of a
\typ{REAL} with 128 bits of accuracy

\noindent \kbd{long} \tet{BIGDEFAULTPREC}: the precision (\kbd{realprec}) of a
...
\fun{void}{initprimetable}{ulong maxp, ulong factorlimit} computes
the prime table \kbd{pari\_PRIMES} (of all primes $p < \kbd{maxp}$);
time complexity is softly linear in \kbd{maxp}. We don't recommend values
...
We recommend to set $\kbd{factorlimit} \leq \kbd{maxp}$; otherwise, the
parameter \kbd{factorlimit} is silently decreased to match \kbd{maxp}.
A generic value of $2^{20}$ is quite sensible for \kbd{factorlimit}, for a
negligible cost at startup (less than 50ms). Only increase this if your own
code will impose \emph{many} trial divisions up to the given limit: PARI
itself uses other algorithms to detect factors larger than $2^{20}$.
...
\fun{GEN}{cgetr}{long b} allocates memory on the stack for a \typ{REAL}
with~\kbd{b} bits of accuracy, and initializes its first codeword. Identical to
\kbd{cgetg(nbits2lg(b),\typ{REAL})}.

\fun{GEN}{cgetc}{long b} allocates memory on the stack for a
\typ{COMPLEX}, whose real and imaginary parts are \typ{REAL}s
allocated with \kbd{cgetr(b)}.
...
\fun{GEN}{cgetr}{long prec} allocates memory on the PARI stack for a \typ{REAL}
of precision~\kbd{prec}, and initializes its first codeword. Identical to
\kbd{cgetg(prec2lg(prec), \typ{REAL})}.

\fun{GEN}{cgetc}{long prec} allocates memory on the PARI stack for a
\typ{COMPLEX}, whose real and imaginary parts are \typ{REAL}s
of precision~\kbd{prec}.

\fun{GEN}{real_1}{long prec} create a \typ{REAL} equal to $1$ with
precision \kbd{prec}.

\fun{GEN}{real_m1}{long prec} create a \typ{REAL} equal to $-1$ to \kbd{prec}
words of accuracy.

\fun{GEN}{real_0_expo}{long e} create a \typ{REAL} equal to $0$ with
exponent $e$.

\fun{GEN}{real_0}{long prec} is a shorthand for \kbd{real\_0\_expo(-prec)}.
It allows to initialize real constants with \kbd{real\_0}, \kbd{real\_1},
\dots \kbd{mplog2}, \kbd{mppi} \dots in a uniform way.
...
\fun{GEN}{itor_lg}{GEN x, long l} converts the \typ{INT}~\kbd{x} to a
\typ{REAL} of length \kbd{l} and return the latter.
Assumes that $\kbd{l} > 2$.

\fun{GEN}{itor}{GEN x, long prec} converts the \typ{INT}~\kbd{x} to a
\typ{REAL} of \kbd{prec} and return the latter. Assumes that $\kbd{prec} > 0$.
...
\fun{GEN}{stor_lg}{long s, long l} converts the \kbd{long}~\kbd{s} into a
\typ{REAL} of length \kbd{l} and return the latter. Assumes that
$\kbd{l} > 2$.

\fun{GEN}{stor}{long s, long prec} converts the \kbd{long}~\kbd{s} into a
\typ{REAL} of precision \kbd{prec} and return the latter. Assumes that
$\kbd{prec} > 0$.
...
\fun{GEN}{utor_lg}{ulong s, long l} converts the \kbd{ulong}~\kbd{s} into a
\typ{REAL} of length \kbd{l} and return the latter. Assumes that
$\kbd{l} > 2$.
...
\fun{GEN}{rtor_lg}{GEN x, long l} converts the \typ{REAL}~\kbd{x} to a
\typ{REAL} of length \kbd{l} and return the latter. If
$\kbd{l} < \kbd{lg(x)}$, round properly. If $\kbd{l} > \kbd{lg(x)}$,
pad with zeroes. Assumes that $\kbd{l} > 2$.

\fun{GEN}{rtor}{GEN x, long prec} converts the \typ{REAL}~\kbd{x} to a
\typ{REAL} of precision \kbd{prec} and return the latter. If
$\kbd{prec} < \kbd{realprec(x)}$, round properly. If $\kbd{prec} >
\kbd{realprec(x)}$, pad with zeroes. Assumes that $\kbd{prec} > 0$.

\noindent The following functions are variants of \tet{mkintn} with
\kbd{unsigned long} arguments (instead of \kbd{unsigned int} for
\kbd{mkintn}). As \kbd{mkintn}, they are mostly used to create large
integer constants in a platform independent way:
...
\bprog
  trunc2nr(x, n-expo(x)+realprec(x)-1)
@eprog
...
\fun{int}{uisnilpotent}{ulong n} return $1$ if every finite group or order $n$
is nilpotent, $0$ otherwise.
...
\fun{GEN}{ZpXQXQ_inv}{GEN b, GEN S, GEN T, GEN p, long e} let
$p$ be a prime \typ{INT} and $b$ be a \kbd{ZpXQXQ} (modulo $S, T, p^e$).
Returns a \kbd{ZpXQXQ} $A$ such that $A\*b \equiv 1 \mod (S, T, p^e)$.

\fun{GEN}{ZpXQXQ_div}{GEN a, GEN b, GEN S, GEN T, GEN q, GEN p, long e} let
$p$ be a prime \typ{INT} and $a$ and $b$ be a \kbd{ZpXQXQ} (modulo $S, T, p^e$).
Returns a \kbd{ZpXQXQ} $c$ such that $c\*b \equiv a \mod (S, T, p^e)$.
The parameter $q$ must be equal to $p^e$.
...
\fun{int}{Z_has_prime_3mod4}{GEN n} return $1$ if and only if the non-zero
integer $n$ has a prime divisor congruent to $3$ modulo $4$.
...
\fun{int}{QM_ishnf}{GEN x} return $1$ if $d x$ is in HNF form for some
positive integer $x$.
...
\kbd{NULL} (handled as $1$). If $c$ is the $k$-th column of the result, it is
essentially $d$ times the $k$-th Lagrange interpolation polynomial: we have
$\sum_j c_j r_i^{j-1} = d \delta_{i=k}$.

This function underlies \kbd{RgV\_polint} when the base field is not
$\Z/p\Z$: it uses $O(n^2)$ scalar operations and is asymptotically slower
than variants using multi-evaluation such as \kbd{FpV\_polint}; it is also
accurate over inexact fields.

A technical variant allows $V$ from \kbd{vandermodeinverseinit\_real}
(complex roots up to conjugation). Then the output need no longer be
a square matrix: only the columns corresponding to the given roots
representative are included. (The others are implied, being their conjugates.)
This allows to reconstruct an algebraic number in $\Q[X]/(T)$ given
$r_1 + r_2$ real and (non-conjugate) complex embeddings only, instead
of $r_1 + 2r_2 = \deg T$.
...
\fun{GEN}{vandermondeinverseinit_real}{GEN R, long r1}
This variant improves on \kbd{vandermondeinverse} in the important
special case where the roots are complex and $T$ has real coefficients, i.e.,
the set of roots is stable under complex conjugation. The argument $r$ is
replaced by a set $R$ of representatives modulo conjugation: the first $r_1$
elements of $R$ are real and the remaining $r_2$ are representatives of pairs
of complex conjugates. This returns the $T'(R_i)$, from which the $T'(r_i)$
can be reconstructed by adding appropriate conjugates.
...
first. If $P$ is already known to be irreducible, use \kbd{ZX\_sturm\_irred}
...
\fun{long}{ZX_sturm_irred}{GEN P} number of real roots of the irreducible
\kbd{ZX} $P$. For efficiency, it is advised to make $P$ primitive first.

\fun{long}{ZX_realroots}{GEN P, long prec} real roots of the nonconstant
squarefree \kbd{ZX} $P$, to precision prec. For efficiency, it is advised to
make $P$ primitive first. If $P$ is known to be irreducible,
\kbd{ZX\_realroots\_irred} will be faster.

\fun{long}{ZX_realroots_irred}{GEN P, long prec} real roots of the
irreducible \kbd{ZX} $P$, to precision prec. For efficiency, it is advised to
make $P$ primitive first.
...
\fun{GEN}{QX_complex_roots}{GEN T, long prec} returns the complex roots of the
\kbd{QX} $T$ at accuracy \kbd{prec}, where real roots are returned as
\typ{REAL}s. More efficient when $T$ is irreducible and primitive. Special
case of \tet{cleanroots}.
...
\fun{GEN}{RgX_homogenous_eval}{GEN P, GEN a, GEN b, long d}
...
\fun{GEN}{fractor_lg}{GEN x, long l} applied to a \typ{FRAC}~\kbd{x}, converts
\kbd{x} into a \typ{REAL} of length~\kbd{l}.

\fun{GEN}{fractor}{GEN x, long prec} applied to a \typ{FRAC}~\kbd{x}, converts
\kbd{x} into a \typ{REAL} of precision~\kbd{prec}.
...
\misctitle{Warning.} Since this is a variadic function, the compiler does
not perform C type promotion on the arguments. You have to ensure that all
the arguments following $n$ are of type \kbd{long}, in particular integer
constants need to be written with the \kbd{L} suffix:
\kbd{mkvecsmalln(2, 1L, 2L)} is correct, but \kbd{mkvecsmalln(2, 1, 2)} is not.
...
expansion in base $2^{32}$ is given by the following $n$ digits. Digits
are \kbd{unsigned int} less than $2^{32}$.
\bprog
  mkintn(3, 0x1U, 0x2U, 0x3U);
@eprog
\noindent returns $2^{64} + 2 \cdot 2^{32} + 3$.

\misctitle{Warning.} Since this is a variadic function, the compiler does
not perform C type promotion on the arguments. You have to ensure that all
the arguments following $n$ are of type \kbd{unsigned int}, in particular
integer constants need to be written with the \kbd{U} suffix as above:
\kbd{mkvecsmalln(3, 1, 2, 3)} would not be correct.
...
for \kbd{gc\_upto}. The complexity is $O(\kbd{prec} / \log(v/u))$
so becomes impractical when $v/u$ is too close to $1$.
...
\fun{GEN}{checkgroup}{GEN gal} check whether \var{gal} is a
...
\fun{GEN}{groupelts_gen}{GEN elts}, where \var{elts} is the list of
elements of a group, returns a small generating set.
...
\fun{long}{group_istrivial}{GEN grp} returns $1$ if the small group
\var{grp} is of order $1$, else $0$.
...
\fun{GEN}{groupgen_export}{GEN gen, long format} convert a small group
given by its generators to another format, as a \typ{STR} describing the group
for the given syntax, see \tet{galoisexport}.
...
analogous to \kbd{snm\_closure(is\_entry(str), mkvecn(...))} but the latter has
