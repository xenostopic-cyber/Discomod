...
\item a binary exponent $e$: a small integer in $[-2^B,2^B[$, where $B = 31$
on a 32-bit machine and 63 otherwise.
...
By definition, \kbd{0.E $n$} returns a real $0$ of decimal exponent $n$, whereas
\kbd{0.} returns a real 0 ``of default precision'' (of decimal exponent
...
To input a \emph{specific} binary floating point number, you can use $m * 2^e
* 1.$, where $m$ is the mantissa: a multiplication by a power of $2$ involves
no rounding. Creating a real $0$ of \emph{binary} exponent $e$ is not obvious
in GP; you can use
\bprog
  realzero(e) = 0. << (e + getlocalbitprec());
@eprog
...
If you type \kbd{?$n$} where $n$ is a number from 1 to 17, you will get the
...
hashtables of identifiers and defaults used by the GP parser. If the argument
is a number $n$, outputs the contents of bucket $n$ (in both tables). Ranges
can be given in the form $m$\kbd{-}$n$ (from bucket $m$ to bucket $n$, \$ =
last bucket). If an identifier or default is given instead of a number or range,
outputs info on the internal structure of the bucket this name occupies
(a \kbd{struct entree} in C). If the range is reduced to a dash ('\kbd{-}'),
outputs statistics about hash buckets usage, for both tables.
...
\subsec{\b{qf}} Stop reading the current \kbd{gp} file or stream. If there are
no remaining file or stream to read, quit GP.
...
\subsec{\b{z} $\{n\}$} Switches \kbd{doctest} on (1) or off (0). If $n$
is explicitly given, set doctest to $n$.
