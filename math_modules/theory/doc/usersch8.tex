...
\fun{long}{lfunthetacost}{GEN ldata, GEN tdom, long m, long bit, long
*extrabit}:
 \kbd{lfunthetacost0} when the first argument is known to be an \kbd{Ldata}.
The \kbd{*extrabit} value is set to an upper estimate for the maximum over
$n$ of the exponent of the $m$-th derivative of $a_n K(nt/\sqrt{N})$, for $t$
in the \kbd{tdom} domain. For an absolute error $2^{-\kbd{bit}}$, the values
summed in $\theta$ should be computed at accuracy $\kbd{bit} +
\kbd{*extrabit}$.
...
\fun{GEN}{gammamellininvrt}{GEN K, GEN s, long bitprec} not stack-clean, but
...
\fun{GEN}{plotrecth}{void *E, GEN(*f)(void*,GEN), long w, GEN a,GEN b, long
