...
on (try \kbd{??readline}): it will be easier to type in examples and
correct typos after you've done that.

Please explore by typing \kbd{?*} (list of all help topics) then trying out
\kbd{?}$n$ and \kbd{??}$n$ for various integers $n$ covering the various
topics.
...
true primes. In fact, it has been checked that the strong
...
exists a large number of arithmetic functions. We have already seen several,
such as \kbd{factor}. Note that \kbd{factor} handles not only integers, but
also univariate polynomials. Type for example \kbd{factor(x\pow 200 - 1)}.
You can also factor a polynomial over a finite field or a number field.
...
\kbd{qfgaussred} (Gauss reduction), \kbd{qfsign} (signature).
Type \kbd{?*} then print the list of linear algebra functions (\kbd{?10}).
...
expected.\smallskip
%
Now type \kbd{?*} then \kbd{?11}. This gives you the list of all transcendental functions.
...
@eprog
This computes a number of quantities related to the elliptic curve defined by
the affine equation
%
...
fields will be worse), one uses so-called \emph{member functions}.
Type \kbd{?.} to get a complete list. Whenever \kbd{ell} appears
...
the change of variable computed above. The point at infinity is represented
as \kbd{[0]} on all elliptic curves; this is the identity for the group law.
...
can be obtained by typing \kbd{polgalois(A)}. The answer \kbd{[8,-1,1,"D(4)"]}
shows that it is equal to $D_4$, the dihedral group with 8 elements,
i.e.~the group of symmetries of a square.
...
\kbd{X Windows} system, Microsoft Windows, macOS), or a \kbd{PostScript}
file ready for the printer, or a Scalable Vector Graphics (SVG) image
ready for the web.

\subsec{Function graphs}

We first decrease drastically the default precision.
\bprog
  \p 9
@eprog\noindent
...
\bprog
  ploth(x = -2*Pi, 2*Pi, sin(x))
@eprog\noindent Straightforward. Let's complicate things:
\bprog
  ploth(x = -2, 2, sin(x^7))
@eprog\noindent
You can see the limitations of the ``straightforward'' mode of plotting:
while the first several cycles of \kbd{sin} around $x = 0$ reach $-1$ and
$1$, the cycles which are closer to the left and right border do not. This is
understandable: PARI is calculating $\sin(X^7)$ at many evenly spaced
points, but these points have no direct relationship to the ``interesting''
points on the graph of this function.  No value close enough to the extrema
are calculated, which leads to wrong turning points in the graph.
To fix this, one may use adaptive steps which are made smaller where the
...
@eprog\noindent
These arrangements break the symmetry of the initial subdivision and
...
graphing in situations when evaluation the function is expensive.
As a moderatly complicated example, let us plot the real part or
$\zeta({1\over 2} + it)$:
\bprog
  ploth(t = 100, 110, real(zeta(0.5+I*t)), /*empty*/, 1000)
@eprog\noindent
(1000 is close to the default for many plotting devices, we specify it
explicitly so that the result does not depend on the output device.)  Try the
recursive plot:
\bprog
  ploth(t = 100, 110, real(zeta(0.5+I*t)), "Recursive")
@eprog\noindent
It takes approximately twice the time.  Now try specifying fewer points,
but make PARI approximate the data by a smooth curve:
\bprog
  ploth(t = 100, 110, real(zeta(0.5+I*t)), "Splines", 100)
@eprog\noindent
This is more than 10 times faster, and the output is practically the same.
How to compare these two outputs?  We will see it shortly.
Right now let us plot both real and complex parts of $\zeta$ on the same graph:
\bprog
  f(t) = my(z = zeta(0.5+I*t)); [real(z),imag(z)];
  ploth(t = 100, 110, f(t), /*empty*/, 1000)
...
\bprog
  ploth(t = 100000, 100004, f(t), "Parametric|Splines|Points_too", 50)
@eprog

\subsec{Low-level plotting}
...
PARI has a power mix of high level plotting function with low level plotting
functions, and these functions can be combined together to obtain many
different effects. We will use a multitude of numeric flags. To simplify
understanding we first give these flags symbolic names:
\bprog
...
@eprog
 ...
 Return back to the graph of $\sin(x^7)$.  Suppose we want to create an
...
\bprog
  plotmove(2, 0,0)
  plotbox(2, 1,1)
@eprog\noindent (nothing is output yet, these commands only fills the virtual
drawing boards with PARI graphic objects).

We now draw the graph inside a subrectangle of this with upper
...
@eprog\noindent Finally, output rectangles 2 and 3 on the same plot, with the
required offsets (counted from upper-left corner):
...
inside this rectangle.  If we use two different
...
but the picture is confusing: which graph is what, and why are there multiple
...
graphs.  And the mystery of multiple boxes is also not hard to solve:
