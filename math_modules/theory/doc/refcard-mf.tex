...
Let $G = \Gamma_0(N)$, $V_k = \QQ[X,Y]_{k-2}$, $L_k = \ZZ[X,Y]_{k-2}$ and
let $\Delta$ be $\text{Div}^0(\PP^1(\QQ))$, generated by \emph{paths}
between cusps of $X_0(N)$ via the identification $[b]-[a] \to $ (path from
$a$ to $b$). In GP, the latter is coded by $[a,b]$ where $a,b$ are
rationals or \kbd{oo} = $(1:0)$.
\smallskip
...
set of $\ZZ[G]$-generators $(g_i)$ of $\Delta$.
\smallskip
