...
for f in build-aux/ltmain.sh m4/libtool.m4 m4/ltoptions.m4 m4/ltsugar.m4 m4/ltversion.m4 m4/lt~obsolete.m4; do
  { wget -nv --timeout=5 -O $f.tmp "https://gitweb.git.savannah.gnu.org/gitweb/?p=gettext.git;a=blob_plain;f=${f}" \
      && mv $f.tmp $f; \
  } || rm -f $f.tmp
done
...
for f in build-aux/config.guess build-aux/config.sub \
         m4/lib-ld.m4 m4/lib-link.m4 m4/lib-prefix.m4 m4/host-cpu-c-abi.m4 m4/asm-underscore.m4 build-aux/config.rpath \
         build-aux/test-driver.diff; do
  { wget -nv --timeout=5 -O $f.tmp "https://gitweb.git.savannah.gnu.org/gitweb/?p=gnulib.git;a=blob_plain;f=${f}" \
      && mv $f.tmp $f; \
  } || rm -f $f.tmp
done
...
for f in m4/ax_cxx_compile_stdcxx.m4; do
  { wget -nv --timeout=5 -O $f.tmp "https://gitweb.git.savannah.gnu.org/gitweb/?p=autoconf-archive.git;a=blob_plain;f=${f}" \
      && mv $f.tmp $f; \
  } || rm -f $f.tmp
done
...
for f in texinfo.tex; do
  g="build-aux/$f"
  { wget -nv --timeout=5 -O $g.tmp "https://gitweb.git.savannah.gnu.org/gitweb/?p=texinfo.git;a=blob_plain;f=doc/${f};hb=refs/tags/texinfo-${TEXINFO_VERSION}" \
      && mv $g.tmp $g; \
  } || rm -f $g.tmp
done
