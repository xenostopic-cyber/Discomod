...
# --- KEY CHANGE: intents are required for normal bots ---
...
def print_header():
    ...
    title = "AI Discord Bot"  # renamed from "AI Selfbot"
...
@bot.event
async def on_ready():
    # Validate owner_id is set to a real user (not the default placeholder)
    ...
    # For a normal bot, owner_id should be the human owner, not the bot itself
    if config["bot"]["owner_id"] == bot.user.id:
        print(f"{Fore.RED}Error: owner_id in config.yaml cannot be the same as the bot's user ID{Style.RESET_ALL}")
        await bot.close()
        sys.exit(1)

    # --- KEY CHANGE: renamed selfbot_id -> bot_id ---
    ...
    print_header()
    print(
        f"AI Bot successfully logged in as {Fore.CYAN}{bot.user.name} ({bot.bot_id}){Style.RESET_ALL}.\n"
    )
...
def should_ignore_message(message):
    # --- KEY CHANGE: replaced selfbot_id check with message.author.bot ---
    # Real bots are identified as bots by Discord, so message.author.bot
    # covers the bot itself AND other bots. No need for a manual ID check.
    return (
...
def is_trigger_message(message):
    ...
    replied_to = (
        ...
        and message.reference.resolved.author.id == bot.bot_id  # renamed from selfbot_id
    )
...
if __name__ == "__main__":
    # --- KEY CHANGE: bot_token=True is implied when using a Bot token ---
