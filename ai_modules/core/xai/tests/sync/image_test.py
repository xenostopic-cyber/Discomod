...
def test_sample_passes_quality(client: Client):
    server.clear_last_image_request()

    client.image.sample(prompt="foo", model="grok-imagine-image-2.0", quality="medium")
...
def test_sample_batch_passes_quality(client: Client):
    server.clear_last_image_request()

    client.image.sample_batch(prompt="foo", model="grok-imagine-image-2.0", n=2, quality="low")
...
def test_sample_omits_quality_by_default(client: Client):
    server.clear_last_image_request()

    client.image.sample(prompt="foo", model="grok-imagine-image-2.0")
...
def test_sample_rejects_invalid_quality(client: Client):
    with pytest.raises(ValueError, match="Invalid image quality"):
        client.image.sample(prompt="foo", model="grok-imagine-image-2.0", quality="ultra")  # type: ignore[arg-type]
...
def test_quality_omitted_from_span_request_attributes_by_default():
    request = _make_generate_request(prompt="foo", model="grok-imagine-image")
    attributes = _make_span_request_attributes(request)
    assert "gen_ai.request.image.quality" not in attributes
...
def test_prepare_passes_quality(client: Client):
    batch_req = client.image.prepare(
        prompt="foo",
...
def test_prepare_omits_quality_by_default(client: Client):
...
def test_prepare_rejects_invalid_quality(client: Client):
    with pytest.raises(ValueError, match="Invalid image quality"):
