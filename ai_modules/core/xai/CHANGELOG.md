...
- **Image Generation Tool Outputs**: Added `Response.image_outputs` for retrieving images generated by the server-side `image_generation` tool as decoded bytes (`output.image`), along with `mime_type`, `data_url`, `image_uuid`, and the originating `tool_call`
...
## [v1.18.0](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.18.0) - 2026-08-13
### Added
...
## [v1.17.1](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.17.1) - 2026-07-14
### Added
- Added `grok-4.5` and `grok-4.5-latest` to the `ChatModel` known-model type literal

## [v1.17.0](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.17.0) - 2026-06-12
### Added
- **Service Tier**: Added `service_tier` parameter to `chat.create()` for requesting priority processing (`"default"` or `"priority"`) at a higher token price. Responses expose a `service_tier` property indicating which tier was actually used

## [v1.16.0](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.16.0) - 2026-06-10
### Added
...
- Added `grok-imagine-image-quality` to the `ImageGenerationModel` known-model type literal and `grok-imagine-video-1.5-preview` to the `VideoGenerationModel` known-model type literal

## [v1.15.0](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.15.0) - 2026-05-30
### Added
- **Context Compaction**: Added `chat.compact_context()` (sync and async) to compact a conversation into an opaque `encrypted_content` representation that can be reused in follow-up requests without sending the full message history. Chat objects also expose `compact()`, which runs compaction on the current conversation and appends the result. The returned `CompactContextResponse` includes `encrypted_content`, `dropped_message_count`, and `usage`.
- Added `grok-build-0.1` to the `ChatModel` known-model type literal

## [v1.14.0](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.14.0) - 2026-05-27
### Added
- **Image Search**: Added `enable_image_search` parameter to `web_search()` to return image results that can be embedded in responses

## [v1.13.0](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.13.0) - 2026-05-16
### Added
...
## [v1.12.2](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.12.2) - 2026-05-07
### Added
- **Reasoning Effort**: Added `"none"` and `"medium"` as accepted `reasoning_effort` values (maps to `EFFORT_NONE` and `EFFORT_MEDIUM`)

## [v1.12.1](https://github.com/xai-org/xai-sdk-python/releases/tag/v1.12.1) - 2026-04-30
### Added
- Added `grok-4.3` and `grok-4.3-latest` to the `ChatModel` known-model type literal
