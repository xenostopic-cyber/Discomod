...
class TestCase(BaseTestCase):
    ...
    def _test_using_loop(self, messages, port):
        ...
        for message, context, result in messages:
            ...
            client.send(
                export(create_evaluation_command(message, context), target_format="wxf")
            )
    ...
    def test_variable_assign(self):
        self._test_using_loop((("a = 2", {}, wl.Null), ("a", {}, 2)), 12345)

    def test_eval_with_context(self):
        self._test_using_loop((("a = 3", {}, wl.Null), ("a", {"a": 2}, 2)), 12346)

    def test_eval_with_delete(self):
        self._test_using_loop(
            (("a = 3", {}, wl.Null), ("del a", {}, wl.Null), ("'a' in globals()", {}, False)),
            12347,
        )
