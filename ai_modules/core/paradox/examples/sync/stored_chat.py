...
def stored_response(client: Client):
    ...
    follow_up_response = chat.sample()
...
def get_stored_response(client: Client):
    ...
    retrieved_responses = client.chat.get_stored_completion(original_response.id)
...
def get_stored_response_batch(client: Client):
    ...
    original_responses = chat.sample_batch(3)
    for index, response in enumerate(original_responses):
        print(f"\nOriginal response {index}: {response.content}")

    retrieved_responses = client.chat.get_stored_completion(response.id)
...
def delete_stored_response(client: Client):
    ...
    deleted_id = client.chat.delete_stored_completion(original_response.id)
    print(f"Deleted response: {deleted_id}")

    try:
        client.chat.get_stored_completion(deleted_id)
...
if __name__ == "__main__":
    ...
    stored_response(client)
    # get_stored_response(client)
    # get_stored_response_batch(client)
    # delete_stored_response(client)
